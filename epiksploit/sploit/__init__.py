...
class Modules(Module, DeauthMixin):
    """ Generic Module proxy class for defining multiple common utility methods for EpikSploit. """
    def preload(self):
        if 'TARGET' not in self.config.keys():
            raise NotImplementedError("Module must have a TARGET option")
